...
const formatStringLine = (content, len, idx, error) => {
...
const formatStringLines = (content, len, from, to, errors) => {
...
const formatBufferLine = (buffer, idx) => {
    let s = '';
...
const formatBufferLines = (buffer, bpl, from, to) => {
    from = from || 0;
    to = to || buffer.length;
...
const formatString = (content, lines, len) => {
...
const formatBuffer = (buffer, lines, len) => {
...
const formatHeader = (idx, type, source, time) => {
    let s = '';
    if (isNumber(idx)) {
...
const formatFile = ({file, lines, len, idx, path, line, column}) => {
