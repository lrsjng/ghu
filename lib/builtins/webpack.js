...
import webpackCb from 'webpack';
import {promisedCbCall, readFile, writeFile, deletePath, pass, passerr} from '../util';
...
export default function action(config, options) {
    return each(obj => {
        return pack(obj.content, obj.source, config, options)
            .then(data => obj.content = data.out);
    });
}
