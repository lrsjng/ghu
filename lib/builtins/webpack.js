...
const pack = (content, filename, config, options) => {
    ...
    const cleanup = (data, throwit = false) => {
        return delete_path(join(dir, baseIn), join(dir, baseOut))
            .catch(() => null)
            .then(() => {
                if (throwit) {
                    throw data;
    ...
    return write_file(join(dir, baseIn), content, true, settings.encoding)
        .then(() => webpack(config))
        .then(stats => {
            ...
            return read_file(join(dir, baseOut), settings.encoding)
                .then(out => {
                    return {stats, out};
                });
        })
