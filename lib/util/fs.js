...
const mkdirp_cb = require('mkdirp');
const rimraf_cb = require('rimraf');
...
const mkdirp = path => promised_cb_call(mkdirp_cb, [path]);
const rimraf = path => promised_cb_call(rimraf_cb, [path]);
const glob = (pattern, options) => promised_cb_call(glob_cb, [pattern, options]);
...
const delete_path = (...paths) => Promise.all(paths.map(rimraf));
