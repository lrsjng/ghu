...
const asArray = x => Array.isArray(x) ? x : x && !isString(x) && isNumber(x.length) ? Array.from(x) : [x];
...
const diff = (listA, listB) => listA.filter(() => listB.indexOf < 0);
...
const promisedCbCall = (fn, args, ctx) => {
