const typeOf = x => Object.prototype.toString.call(x); // eslint-disable-line prefer-reflect

export const isNumber = x => typeOf(x) === '[object Number]';
export const isString = x => typeOf(x) === '[object String]';
export const isArray = x => Array.isArray(x);
export const isBuffer = x => Buffer.isBuffer(x);
export const isFn = x => typeOf(x) === '[object Function]';
export const asFn = x => isFn(x) ? x : () => x;
export const asArray = x => isArray(x) ? x : x && !isString(x) && isNumber(x.length) ? Array.from(x) : [x];

export const runConcurrent = fns => Promise.all(fns.map(fn => asFn(fn)()));
